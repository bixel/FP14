...
Der Modulationsgrad $m$ kann mit Kenntnis der minimalen und maximalen
...
von $U_\text{max}=\input{build/UtPlusM.tex}$ und einer minimalen Amplitude
von $U_\text{min}=\input{build/UtMinusM.tex}$
\begin{equation*}
...
entsprechenden Amplituden von $U_\text{T} = \input{build/UT.tex}$,
$U_1 = \input{build/U1.tex}$ und $U_2 = \input{build/U2.tex}$
der Modulationsgrad
\begin{equation*}
...
$\Delta T = \input{build/delta_t.tex}$ pro Periode.
Die hierfür verwendeten Messwerte sind in Tabelle \ref{tab:delta_ts}
aufgeführt.
Die Trägerfrequenz beträgt $f_\text{T} = \input{build/omega_t.tex}$, woraus
die Periodendauer $T_\text{T} = 1 / f_\text{T} = \input{build/t_t.tex}$
folgt.
Mit
\begin{equation}
\label{eq:}
