...
alpha_e1 = [0 for i in range(len(a_long))]
...
for i in range(len(a_long)):
	alpha_e[i] = -20*log(ufloat(a_long[i],0.5) / ufloat(a_short[i],0.5))

for i in range(len(a_long)):
	alpha_e1[i] = alpha_e[i].std_dev
...
plt.close(fig)
fig = plt.figure()
plt.errorbar(xval, alpha, yerr=alpha_e1, fmt="bo")
